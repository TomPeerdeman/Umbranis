<?php
	...
	class KlantoverzichtController extends BaseController{
...
<?php
				...
				$res = DB::$db->query("SELECT * FROM users where username =" . DB::$db->quote($_GET['id'])."");
				if(!$res){
...
<?php				$res = DB::$db->query("SELECT * FROM orders 
										where user_id =" . DB::$db->quote($_GET['id'])."
										ORDER BY date DESC");
					if(!$res){
						echo "<p>Er is een fout in de database opgetreden!</p></div>";
						return;
