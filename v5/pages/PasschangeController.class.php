	...
	class PasschangeController extends BaseController{
		...
		public function handleForm(){
			...
			if($this->check){	
				...
				$res = DB::$db->query("SELECT password_salt FROM users WHERE username=" . DB::$db->quote($this->user->username) . " LIMIT 1");
				if(!$res){
				...
				if(($res->rowCount() != 1)){
					...
					if(!DB::$db->query("UPDATE users SET password='" . $passhash . "', password_salt='" . $salt . "' 
...
<?php			if (!$this->user->is_member()){
			...
			if(count($this->errors) > 0){
				...
				echo "U wordt automatisch doorgestuurd na 5 seconden gebeurt dit niet klik dan <a href=\"?p=account\">hier</a>.</p>";
				echo "<meta http-equiv=\"refresh\" content=\"5;url=?p=account\" />";
			}
		}
